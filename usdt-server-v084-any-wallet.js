...
const USDT_RECEIVER = String(process.env.USDT_RECEIVER || '0xbf94435dc4e7233c50691e6bcabf52b778be4751').trim();
  ...
  if (req.method === 'POST' && req.url === '/usdt-verify-server') {
    ...
    req.on('end', async () => {
      ...
      try {
        ...
        return json(res, status, { ok: false, error: message, verifierVersion: 'v084', balanceChangedByServer: false });
      }
    });
    return;
  }
...
server.listen(PORT, '0.0.0.0', () => console.log(`ESCROW X USDT verifier v084 ANY-WALLET listening on port ${PORT}`));
