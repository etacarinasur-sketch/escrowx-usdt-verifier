  ...
  try{
    ...
    if(!destinationMatches.length){
      ...
      const diagnostic={
        ...
        configuredReceiver:normalizeAddress(USDT_RECEIVER),
        usdtContract:contract,
        transferTopic:TRANSFER_TOPIC,
        receiptLogCount:receiptLogs.length,
        usdtLogCount:usdtLogs.length,
        transferTopicLogCount:transferTopicLogs.length,
        parsedTransferCandidates:transfers.slice(0,20).map(x=>({from:x.from,to:x.to,amount:formatUnits(x.units,USDT_DECIMALS),logIndex:x.logIndex})),
        receiptLogs:logDiagnostics
      };
    ...
    return {
      ok:true,realConnector:true,verifierVersion:'v087',verificationScope:'BLOCKCHAIN_PROOF_ONLY',walletPolicy:'ANY_BSC_SENDER',financialCredit:'ESCROW_X_ONLY',
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&req.url==='/health'){
    try{
      ...
      const chainId=Number.parseInt(chainIdHex,16);
      return json(res,200,{ok:chainId===BSC_CHAIN_ID,rpcConnected:true,chainId,network:'BNB Smart Chain Mainnet',tokenContract:USDT_CONTRACT,receiver:USDT_RECEIVER,verifierVersion:'v087',verificationScope:'BLOCKCHAIN_PROOF_ONLY',walletPolicy:'ANY_BSC_SENDER',balanceChangedByServer:false});
  ...
  if(req.method==='POST'&&req.url==='/usdt-verify-server'){
    ...
    req.on('end',async()=>{
      ...
      try{
        ...
        return json(res,status,{ok:false,error:message,verifierVersion:'v087',balanceChangedByServer:false});
      }
    });
    return;
  }
