...
async function verifyUSDT(body){
  ...
  try{
    ...
    if(!destinationMatches.length){
      const seen=transfers.slice(0,10).map(x=>({from:x.from,to:x.to,amount:formatUnits(x.units,USDT_DECIMALS),logIndex:x.logIndex}));
      const detail=JSON.stringify({submittedFrom:wantedFrom,submittedReceiver:wantedTo,transferCandidates:seen});
    ...
    return {
      ok:true,realConnector:true,verifierVersion:'v086',verificationScope:'BLOCKCHAIN_PROOF_ONLY',walletPolicy:'ANY_BSC_SENDER',financialCredit:'ESCROW_X_ONLY',
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&req.url==='/health'){
    try{
      ...
      const chainId=Number.parseInt(chainIdHex,16);
      return json(res,200,{ok:chainId===BSC_CHAIN_ID,rpcConnected:true,chainId,network:'BNB Smart Chain Mainnet',tokenContract:USDT_CONTRACT,receiver:USDT_RECEIVER,verifierVersion:'v086',verificationScope:'BLOCKCHAIN_PROOF_ONLY',walletPolicy:'ANY_BSC_SENDER',balanceChangedByServer:false});
  ...
  if(req.method==='POST'&&req.url==='/usdt-verify-server'){
    ...
    req.on('end',async()=>{
      ...
      try{
        ...
        return json(res,status,{ok:false,error:message,verifierVersion:'v086',balanceChangedByServer:false});
      }
    });
    return;
  }
...
server.listen(PORT,'0.0.0.0',()=>console.log(`ESCROW X USDT verifier v086 TRANSFER-MATCH REPAIRED listening on port ${PORT}`));
