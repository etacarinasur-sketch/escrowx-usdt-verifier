...
async function verifyUSDT(body){
  ...
  try{
    ...
    const expectedFromTopic=padAddressTopic(wantedFrom).toLowerCase();
    const expectedToTopic=padAddressTopic(wantedTo).toLowerCase();
    const transfers=[];
    const receiptLogs=Array.isArray(receipt.logs)?receipt.logs:[];
    const allUsdtLogs=[];

    // v089: explicit topic comparison. Standard ERC-20 Transfer events put
    // topic[1]=indexed from and topic[2]=indexed to, each as a 32-byte,
    // left-padded address. We compare both the raw padded topic and the
    // decoded last-20-byte address. No wallet/financial state is changed.
    for(const log of receiptLogs){
      ...
      const decodedFrom=topicAddress(topic1);
      const decodedTo=topicAddress(topic2);
      const record={
        logIndex:String(log?.logIndex||''),
        topic0,topic1,topic2,
        decodedFrom,decodedTo,
        data:String(log?.data||''),
        rawFromMatches:topic1===expectedFromTopic,
        rawToMatches:topic2===expectedToTopic,
        decodedFromMatches:decodedFrom===wantedFrom,
        decodedToMatches:decodedTo===wantedTo
      };
      allUsdtLogs.push(record);
      if(topic0!==TRANSFER_TOPIC||topics.length<3)continue;
      let units;
      try{units=hexToBigInt(log.data||'0x0');}catch(_){continue;}
      transfers.push({from:decodedFrom,to:decodedTo,units,logIndex:Number.parseInt(String(log.logIndex||'0x0'),16)||0,record});
    }

    const fromMatches=transfers.filter(x=>x.record.rawFromMatches||x.record.decodedFromMatches);
    const destinationMatches=fromMatches.filter(x=>x.record.rawToMatches||x.record.decodedToMatches);
    const amountMatches=destinationMatches.filter(x=>x.units===requestedUnits);

    if(!destinationMatches.length){
      const diagnostic={
        submittedFrom:wantedFrom,
        submittedReceiver:wantedTo,
        expectedFromTopic,
        expectedToTopic,
        receiptLogCount:receiptLogs.length,
        usdtLogCount:allUsdtLogs.length,
        transferTopicLogCount:transfers.length,
        transferTopic:TRANSFER_TOPIC,
        usdtLogs:allUsdtLogs.slice(0,12)
      };
    ...
    return {
      ok:true,realConnector:true,verifierVersion:'v090',verificationScope:'BLOCKCHAIN_PROOF_ONLY',walletPolicy:'ANY_BSC_SENDER',financialCredit:'ESCROW_X_ONLY',
...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&req.url==='/health'){
    try{
      ...
      const chainId=Number.parseInt(chainIdHex,16);
      return json(res,200,{ok:chainId===BSC_CHAIN_ID,rpcConnected:true,chainId,network:'BNB Smart Chain Mainnet',tokenContract:USDT_CONTRACT,receiver:USDT_RECEIVER,verifierVersion:'v090',verificationScope:'BLOCKCHAIN_PROOF_ONLY',walletPolicy:'ANY_BSC_SENDER',balanceChangedByServer:false});
  ...
  if(req.method==='POST'&&req.url==='/usdt-verify-server'){
    ...
    req.on('end',async()=>{
      ...
      try{
        ...
        return json(res,status,{ok:false,error:message,verifierVersion:'v090',balanceChangedByServer:false});
      }
    });
    return;
  }
...
server.listen(PORT,'0.0.0.0',()=>console.log(`ESCROW X USDT verifier v090 TRANSFER TOPIC REPAIR listening on port ${PORT}`));
